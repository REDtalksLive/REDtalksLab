...
##############################################
#
resource "random_id" "service_chat_api_id" {
  count = length(var.service_chat_api.services)
  keepers = {
    name = var.service_chat_api.services[count.index].name
  }
  byte_length = 8
}
...
##############################################
#
resource "esxi_guest" "ServiceChatApi" {
  count = length(var.service_chat_api.services)
  #for_each = var.service_chat_api.services

  guest_name = "${var.service_chat_api.name_prefix}-${random_id.service_chat_api_id[count.index].keepers.name}-${random_id.service_chat_api_id[count.index].hex}"
  disk_store = "ds1"
  ...
  provisioner "file" {
    content     = templatefile("${path.module}/scripts/systemd-setup-envoy.sh", { service_id = "${var.service_chat_api.services[count.index].name}-${count.index}" })
    ...
    content = templatefile("${path.module}/config_templates/consul_client_sm-envoy.hcl",
      {
        ...
        service_id            = "${var.service_chat_api.services[count.index].name}-${count.index}"
        service_name          = var.service_chat_api.services[count.index].name,
        service_port          = var.service_chat_api.services[count.index].port,
        gossip_key            = random_id.gossip_key.b64_std,
        sidecar_config        = var.service_chat_api.services[count.index].sidecar
      }
    )
  ...
  provisioner "remote-exec" {
    inline = [
      "echo ${var.vm_guest_password} | sudo -S hostnamectl set-hostname ${var.service_chat_api.services[count.index].name}-${count.index}",
